...
import NonDevUserException from '../exceptions/NonDevUserException';
import { getRequestAuthToken } from './getRequestAuthToken';

async function devAuthMiddleware(request: Request, _response: Response, next: NextFunction) {
              ...
              if (user.status === 'ENABLED') {
                if (blacklistTokens.tokenExpired(verificationResponse.expiryInSec,
                  ...
                  if (
                    ...
                    if (verificationResponse.devUser) {
                      next();
                    } else {
                      SysLog.error('Authentication Failed For Non Dev User Id :', id);
                      next(new NonDevUserException(user));
                    }
                  }
                }
              } else {
